...
public class SpawnEggItem1 extends Item {
   ...
   public Optional<MobEntity> dgrurbMobEntity(PlayerEntity player, MobEntity mobEntity, EntityType<? extends MobEntity> entityType, World worldIn, Vector3d vector3d, ItemStack stack) {
      if (!this.hasType(stack.getTag(), entityType)) {
         return Optional.empty();
      } else {
         MobEntity mobentity;
         if (mobEntity instanceof AgeableEntity) {
            mobentity = ((AgeableEntity)mobEntity).createChild((AgeableEntity)mobEntity);
         } else {
            mobentity = entityType.create(worldIn);
         }
         ...
         if (mobentity == null) {
            ...
            if (!mobentity.isChild()) {
               ...
               mobentity.setLocationAndAngles(vector3d.getX(), vector3d.getY(), vector3d.getZ(), 0.0F, 0.0F);
