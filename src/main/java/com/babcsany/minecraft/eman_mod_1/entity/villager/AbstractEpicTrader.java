package com.babcsany.minecraft.eman_mod_1.entity.villager;

import com.babcsany.minecraft.eman_mod_1.entity.villager.trades.OplikuxiTrades;
import com.babcsany.minecraft.eman_mod_1.entity.villager.trigger.CriteriaTriggers1;
import com.babcsany.minecraft.eman_mod_1.entity.villager.trades.EpicTraderTrades;
...
public abstract class AbstractEpicTrader extends AgeableEntity implements INPC, IMerchant {
   private static final DataParameter<Integer> SHAKE_HEAD_TICKS = EntityDataManager.createKey(AbstractEpicTrader.class, DataSerializers.VARINT);
   @Nullable
   ...
   public AbstractEpicTrader(EntityType<? extends AbstractEpicTrader> type, World worldIn) {
      super(type, worldIn);
   ...
   public void onTrade(MerchantOffer offer) {
      ...
      if (this.customer instanceof ServerPlayerEntity) {
         CriteriaTriggers1.ZOMBIE_TRADER_TRADE.test((ServerPlayerEntity)this.customer, this, offer.getSellingStack());
      }
    ...
    */
   protected void addTrades(MerchantOffers givenMerchantOffers, EpicTraderTrades.ITrade[] newTrades, int maxNumbers) {
      ...
      for(Integer integer : set) {
         EpicTraderTrades.ITrade zombieTraderTrades$itrade = newTrades[integer];
   ...
   protected void addTrades1(MerchantOffers givenMerchantOffers, OplikuxiTrades.ITrade[] newTrades, int maxNumbers) {
      Set<Integer> set = Sets.newHashSet();
      if (newTrades.length > maxNumbers) {
         while(set.size() < maxNumbers) {
            set.add(this.rand.nextInt(newTrades.length));
      ...
      for(Integer integer : set) {
         OplikuxiTrades.ITrade zombieTraderTrades$itrade = newTrades[integer];
         MerchantOffer merchantoffer = zombieTraderTrades$itrade.getOffer(this, this.rand);
         if (merchantoffer != null) {
            givenMerchantOffers.add(merchantoffer);
         }
