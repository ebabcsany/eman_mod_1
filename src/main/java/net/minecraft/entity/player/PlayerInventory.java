...
public class PlayerInventory implements IInventory, INameable {
   ...
   public void func_234563_a_(DamageSource damageSource, float p_234563_2_) {
      if (!(p_234563_2_ <= 0.0F)) {
         ...
         for(int i = 0; i < this.armorInventory.size(); ++i) {
            ...
            if ((!damageSource.isFireDamage() || !itemstack.getItem().isBurnable()) && itemstack.getItem() instanceof ArmorItem) {
               int j = i;
