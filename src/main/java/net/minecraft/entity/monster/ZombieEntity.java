...
public class ZombieEntity extends MonsterEntity {
   ...
   private static final AttributeModifier BABY_SPEED_BOOST = new AttributeModifier(BABY_SPEED_BOOST_ID, "Baby speed boost", 0.15D, AttributeModifier.Operation.MULTIPLY_BASE);
   ...
   public static AttributeModifierMap.MutableAttribute func_234342_eQ_() {
      return MonsterEntity.func_234295_eP_().createMutableAttribute(Attributes.FOLLOW_RANGE, 47.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, (double)0.10F).createMutableAttribute(Attributes.ATTACK_DAMAGE, 4.0D).createMutableAttribute(Attributes.ARMOR, 2.0D).createMutableAttribute(Attributes.ZOMBIE_SPAWN_REINFORCEMENTS);
   }
   ...
   public void setBreakDoorsAItask(boolean enabled) {
      if (this.canBreakDoors()) {
         if (this.isBreakDoorsTaskSet != enabled) {
            ...
            if (enabled) {
               this.goalSelector.addGoal(12, this.breakDoor);
            } else {
   ...
   public void onKillEntity(LivingEntity entityLivingIn) {
      ...
      if ((this.world.getDifficulty() == Difficulty.NORMAL || this.world.getDifficulty() == Difficulty.HARD) && entityLivingIn instanceof VillagerEntity) {
         ...
         zombievillagerentity.onInitialSpawn(this.world, this.world.getDifficultyForLocation(zombievillagerentity.getPosition()), SpawnReason.CONVERSION, new GroupData(false, true), (CompoundNBT)null);
